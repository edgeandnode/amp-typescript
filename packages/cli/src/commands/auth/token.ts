...
const audienceFlag = Flag.String("audience").pipe(
  Flag.withAlias("a"),
  Flag.withDescription("URLs that are valid to use the generated access token. Becomes the JWT aud value"),
  Flag.atLeast(0)
)
...
const handleTokenCommand = Effect.fnUntraced(
  function* ({ audience, duration }: { readonly audience: Array<string>; readonly duration: Models.TokenDuration }) {
    ...
    yield* auth.verifyAccessToken(Redacted.make(response.token), response.iss).pipe(
      Effect.catch(
        Effect.fnUntraced(function* (error) {
          const userMessage = AuthError.getUserMessage(error)
          const userSuggestion = AuthError.getUserSuggestion(error)
          const errorMessage = ["Failed to verify the signed token.", userMessage, userSuggestion].join("\n")
          yield* Console.error(errorMessage)
          return yield* new TokenCommandError({})
        })
      )
...
export const TokenCommand = Command.make("token", { audience: audienceFlag, duration: durationArg }).pipe(
  Command.withDescription("Generates an access token (Bearer JWT) to be used by your applictaion to interact with Amp"),
