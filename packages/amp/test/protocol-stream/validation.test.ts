...
const makeBlockRange = (network: string, start: number, end: number, hash: string, prevHash?: string): BlockRange => ({
...
describe("validateNetworks", () => {
  it.effect("allows first batch with any networks", ({ expect }) =>
    Effect.gen(function* () {
      const incoming = [makeBlockRange("eth", 0, 10, HASH_A), makeBlockRange("polygon", 0, 10, HASH_B)]
  ...
  it.effect("allows consistent networks across batches", ({ expect }) =>
    Effect.gen(function* () {
      const previous = [makeBlockRange("eth", 0, 10, HASH_A), makeBlockRange("polygon", 0, 10, HASH_B)]
      const incoming = [
  ...
  it.effect("rejects duplicate networks in batch", ({ expect }) =>
    Effect.gen(function* () {
      const incoming = [makeBlockRange("eth", 0, 10, HASH_A), makeBlockRange("eth", 0, 10, HASH_B)]
  ...
  it.effect("rejects network count change", ({ expect }) =>
    Effect.gen(function* () {
      ...
      const incoming = [makeBlockRange("eth", 11, 20, HASH_B, HASH_A), makeBlockRange("polygon", 0, 10, HASH_C)]
  ...
  it.effect("rejects unexpected network", ({ expect }) =>
    Effect.gen(function* () {
      const previous = [makeBlockRange("eth", 0, 10, HASH_A)]
      const incoming = [makeBlockRange("polygon", 0, 10, HASH_B)]
      const result = yield* Effect.flip(validateNetworks(previous, incoming))
